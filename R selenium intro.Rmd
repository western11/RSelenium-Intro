...
```{r,eval=FALSE}
clean_review <- data.frame()
for(i in 1:nrow(review_filter)){
  message("Cleaning",i,"/",nrow(review_filter))
  temp_clean <- textcleaner(review_filter$Review[i])
  
  clean_review <-  rbind(clean_review,temp_clean)
}

```

```{r,eval=FALSE}
saveRDS(clean_review,"review_clean.rds")
```
